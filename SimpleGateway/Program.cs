...
try
{
    using (var scope = app.Services.CreateScope())
    {
        ...
        try 
        {
            ...
            Console.WriteLine("=== STRUCTURED CONVERSATION TABLE VERIFICATION ===");
            try
            {
                var structuredConvCount = context.StructuredConversations.Count();
                Console.WriteLine($"✅ StructuredConversations table EXISTS: {structuredConvCount} records found");
            }
            catch (Exception tableEx)
            {
                Console.WriteLine($"❌ StructuredConversations table verification FAILED: {tableEx.Message}");
                Console.WriteLine($"Exception type: {tableEx.GetType().Name}");
                
                // AGGRESSIVE FALLBACK: Manual table creation for Railway deployment issues
                Console.WriteLine("🔧 FORCING manual StructuredConversations table creation...");
                try
                {
                    var createTableSql = @"
                        DROP TABLE IF EXISTS ""StructuredConversations"";
                        CREATE TABLE ""StructuredConversations"" (
                    ...
                    Console.WriteLine("✅ StructuredConversations table FORCE CREATED manually");
                ...
                catch (Exception manualEx)
                {
                    ...
                    Console.WriteLine($"Manual exception type: {manualEx.GetType().Name}");
                }
            }
