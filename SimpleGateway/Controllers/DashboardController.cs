...
namespace SimpleGateway.Controllers
{
    public class DashboardController : Controller
    {
        ...
        public IActionResult FinalAssessment(string performerUsername)
        {
            var currentUser = HttpContext.Session.GetString("username");
            var currentRole = HttpContext.Session.GetString("role");

            if (string.IsNullOrEmpty(currentUser) || string.IsNullOrEmpty(currentRole))
            {
                return RedirectToAction("Index", "Home");
            }

            // Emergency table creation for Final Sign-offs
            try
            {
                _context.Database.ExecuteSqlRaw(@"
                ...
                ");
                Console.WriteLine("Final Sign-off: Emergency table creation completed successfully");
            }
            catch (Exception createEx)
            {
                Console.WriteLine($"Final Sign-off: Emergency table creation issue (continuing): {createEx.Message}");
            }

            // Get or create the final sign-off record
            var existingSignOff = _context.FinalSignOffs
                .FirstOrDefault(f => f.PerformerUsername == performerUsername);

            if (existingSignOff == null)
            {
                existingSignOff = new FinalSignOffModel
                {
                    PerformerUsername = performerUsername,
                    CreatedAt = DateTime.UtcNow,
                    UpdatedAt = DateTime.UtcNow
                };
                _context.FinalSignOffs.Add(existingSignOff);
                _context.SaveChanges();
            }

            // Field-Level Permissions following the established pattern
            ViewBag.CanSignPerformerDeclaration = (currentRole == "performer" && currentUser == performerUsername && !existingSignOff.PerformerDeclarationSigned);
            ViewBag.CanSignSupervisorDeclaration = ((currentRole == "supervisor" || currentRole == "admin" || currentRole == "superuser") && !existingSignOff.SupervisorDeclarationSigned);
            ViewBag.CanSignAdvisorDeclaration = ((currentRole == "advisor" || currentRole == "admin" || currentRole == "superuser") && !existingSignOff.AdvisorDeclarationSigned);

            // Standard ViewBag properties
            ...
            var performer = _context.Users.FirstOrDefault(u => u.Username == performerUsername);
            ViewBag.PerformerName = performer?.DisplayName ?? performerUsername;

            return View("Performer/FinalAssessment", existingSignOff);
        }

        [HttpPost]
        public IActionResult SignDeclaration(string performerUsername, string declarationType)
        {
            var currentUser = HttpContext.Session.GetString("username");
            var currentRole = HttpContext.Session.GetString("role");
            ...
            var signOff = _context.FinalSignOffs
                .FirstOrDefault(f => f.PerformerUsername == performerUsername);

            if (signOff == null)
            {
                TempData["ErrorMessage"] = "Final sign-off record not found.";
                return RedirectToAction("FinalAssessment", new { performerUsername });
            }

            // Validate permissions and process signature
            switch (declarationType.ToLower())
            {
                case "performer":
                    if (currentRole == "performer" && currentUser == performerUsername && !signOff.PerformerDeclarationSigned)
                    {
                        signOff.PerformerDeclarationSigned = true;
                        signOff.PerformerSignedBy = currentUser;
                        signOff.PerformerSignedAt = DateTime.UtcNow;
                        TempData["SuccessMessage"] = "Performer declaration signed successfully.";
                    }
                    else
                    {
                        TempData["ErrorMessage"] = "You are not authorized to sign the performer declaration.";
                    }
                    break;

                case "supervisor":
                    if ((currentRole == "supervisor" || currentRole == "admin" || currentRole == "superuser") && !signOff.SupervisorDeclarationSigned)
                    {
                        signOff.SupervisorDeclarationSigned = true;
                        signOff.SupervisorSignedBy = currentUser;
                        signOff.SupervisorSignedAt = DateTime.UtcNow;
                        TempData["SuccessMessage"] = "Supervisor declaration signed successfully.";
                    }
                    else
                    {
                        TempData["ErrorMessage"] = "You are not authorized to sign the supervisor declaration.";
                    }
                    break;

                case "advisor":
                    if ((currentRole == "advisor" || currentRole == "admin" || currentRole == "superuser") && !signOff.AdvisorDeclarationSigned)
                    {
                        signOff.AdvisorDeclarationSigned = true;
                        signOff.AdvisorSignedBy = currentUser;
                        signOff.AdvisorSignedAt = DateTime.UtcNow;
                        TempData["SuccessMessage"] = "Advisor declaration signed successfully.";
                    }
                    else
                    {
                        TempData["ErrorMessage"] = "You are not authorized to sign the advisor declaration.";
                    }
                    break;

                default:
                    TempData["ErrorMessage"] = "Invalid declaration type.";
                    return RedirectToAction("FinalAssessment", new { performerUsername });
            }
        ...
        {
            // Redirect to the new FinalAssessment method
            return RedirectToAction("FinalAssessment", new { performerUsername });
        }
